...
/*
Package asynq_learn provides a framework for Redis based distrubted task queue.

Asynq uses Redis as a message broker. To connect to redis,
specify the connection using one of RedisConnOpt types.

	redisConnOpt = asynq_learn.RedisClientOpt{
	    Addr:     "127.0.0.1:6379",
	    Password: "xxxxx",
	    DB:       2,
	}
...
The Client is used to enqueue a task.
	...
	client := asynq_learn.NewClient(redisConnOpt)
	...
	task := asynq_learn.NewTask("example", b)

	// Enqueue the task to be processed immediately.
	info, err := client.Enqueue(task)

	// Schedule the task to be processed after one minute.
	info, err = client.Enqueue(t, asynq_learn.ProcessIn(1*time.Minute))

The Server is used to run the task processing workers with a given
handler.

	srv := asynq_learn.NewServer(redisConnOpt, asynq_learn.Config{
	    Concurrency: 10,
	})
...
Example of a type that implements the Handler interface.
	...
	func (h *TaskHandler) ProcessTask(ctx context.Context, task *asynq_learn.Task) error {
	    switch task.Type {
	    case "example":
